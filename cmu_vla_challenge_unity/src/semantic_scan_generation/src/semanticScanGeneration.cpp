...
int main(int argc, char** argv)
{
  ...
  // change register_scan to sensor_scan
  ...
  while (status)
  {
    ...
    try {
      transformStamped = tfBuffer.lookupTransform("camera", "map", ros::Time(0));
    ...
    if (imageInit && newLaserCloud) {
      ...
      for (int i = 0; i < laserCloudSize; i++) {
        ...
        if (horiPixelID >= 0 && horiPixelID < imageWidth && vertPixelID >= 0 && vertPixelID < imageHeight) {
          ...
          // tf2를 사용하여 포인트를 /camera 프레임으로 변환합니다.
          geometry_msgs::PointStamped laser_point, transformed_point;
          laser_point.header.frame_id = "map";
          laser_point.header.stamp = ros::Time();
          laser_point.point.x = laserCloud->points[i].x;
          laser_point.point.y = laserCloud->points[i].y;
          laser_point.point.z = laserCloud->points[i].z;
      ...
      laserCloudOut.header.frame_id = "camera";
